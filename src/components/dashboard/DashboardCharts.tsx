...
const DashboardCharts = ({
  ...
  const causesByValue = orders.filter(o => o.fees && o.fees.length > 0).flatMap(o => o.fees!).reduce((acc, fee) => {
  ...
  const resolutionData = [{
    ...
    name: "Custo confirmado",
    value: confirmedCostValue,
    color: COLORS.danger
  }, {
    name: "Em suporte",
    value: supportOpenValue,
    color: COLORS.warning
  }].filter(d => d.value > 0);
  ...
  return <div className="space-y-6">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        <Card className="p-6">
          ...
          <p className="text-sm text-muted-foreground mb-4">
            {reconciliationTotal > 0 && reconciledAutomatically > 0 && `Taxa de conferência automática: ${Math.round(reconciledAutomatically / reconciliationTotal * 100)}%`}
          </p>
          {reconciliationTotal > 0 ? <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                <Pie data={reconciliationStatusData} cx="50%" cy="50%" labelLine={false} label={({
              name,
              percent
            }) => `${name} ${Number.isFinite(percent) ? (percent * 100).toFixed(0) : 0}%`} outerRadius={100} fill="#8884d8" dataKey="value">
                  {reconciliationStatusData.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.color} />)}
                </Pie>
                <Tooltip />
              </PieChart>
            </ResponsiveContainer> : <div className="h-[300px] flex items-center justify-center text-muted-foreground text-sm">
        ...
        {causesByValue.length > 0 && <Card className="p-6">
            <h3 className="text-lg font-semibold text-foreground mb-2">
        ...
        {/* Resolution Results */}
        {resolutionData.length > 0 && <Card className="p-6">
            <h3 className="text-lg font-semibold text-foreground mb-2">
            ...
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                <Pie data={resolutionData} cx="50%" cy="50%" labelLine={false} label={({
              name,
              value
            }) => `${name}: R$ ${value.toFixed(2)}`} outerRadius={100} fill="#8884d8" dataKey="value">
                  {resolutionData.map((entry, index) => <Cell key={`cell-${index}`} fill={entry.color} />)}
                </Pie>
      ...
      {/* Timeline Chart */}
      {timelineData.length > 0 && <Card className="p-6">
          <h3 className="text-lg font-semibold text-foreground mb-2">
          ...
          <ResponsiveContainer width="100%" height={300}>
            ...
            minimumFractionDigits: 2
              ...
              <Legend />
              <Line type="monotone" dataKey="previsto" stroke={COLORS.ok} strokeWidth={2} name="Previsto" />
              <Line type="monotone" dataKey="recebido" stroke={COLORS.difference} strokeWidth={2} name="Recebido" />
            </LineChart>
          </ResponsiveContainer>
        </Card>}
      ...
      <Card className="p-6 bg-gradient-to-r from-primary/5 to-primary/10 border-primary/20">
        <div className="space-y-3">
          ...
          {recoveredValue > 0 && <p className="text-sm text-foreground">
              💸 <strong>Você já recuperou R$ {recoveredValue.toLocaleString('pt-BR', {
              minimumFractionDigits: 2
            })}</strong> de valores que passariam despercebidos.
            </p>}
          {timeSaved > 0 && <p className="text-sm text-foreground">
              🕒 <strong>Economizou {timeSaved} horas</strong> de trabalho manual neste período.
            </p>}
          {resolvedCount > 0 && totalWithDifference > 0 && <p className="text-sm text-foreground">
          ...
          {supportOpenValue > 0 && <p className="text-sm text-foreground">
              ⚠️ <strong>R$ {supportOpenValue.toLocaleString('pt-BR', {
              minimumFractionDigits: 2
            })} ainda em suporte aberto</strong>, acompanhe o andamento.
            </p>}
          {confirmedCostValue > 0 && <p className="text-sm text-foreground">
              ❤️ <strong>R$ {confirmedCostValue.toLocaleString('pt-BR', {
              minimumFractionDigits: 2
            })} confirmados como custo</strong> — use esse dado para renegociar taxas.
            </p>}
        </div>
      </Card>
    </div>;
