...
import { TrendingUp, Clock, CheckCircle, XCircle, AlertCircle, MessageSquare, TrendingDown, Info, Target, Lightbulb, ArrowRight, Search, FileText, BarChart3, DollarSign, RefreshCcw } from "lucide-react";
...
const DashboardCharts = ({
  ...
  const differencesByType = orders.filter(o => o.fees && o.fees.length > 0).flatMap(o => o.fees!).reduce((acc, fee) => {
  ...
  // ========== Timeline data ==========
  const timelineData = orders.reduce((acc, order) => {
  ...
  return <div className="space-y-8">
      ...
      <section>
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-xl font-bold text-foreground">Resumo Analítico</h2>
        </div>
          ...
          <Card className="p-5 bg-danger-light border-danger-border">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-2xl font-bold text-foreground">
                  R$ {confirmedCostValue.toLocaleString('pt-BR', {
                  minimumFractionDigits: 2
                })}
                </p>
                <p className="text-xs text-muted-foreground mt-1">
          ...
          <Card className="p-5 bg-orange-500/10 border-orange-500/20">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-2xl font-bold text-foreground">
                  R$ {supportOpenValue.toLocaleString('pt-BR', {
                  minimumFractionDigits: 2
                })}
                </p>
                <p className="text-xs text-muted-foreground mt-1">
      ...
      <section>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {differencesByType.length > 0 ? <Card className="p-6">
              <CardHeader className="p-0 mb-4">
                ...
                <CardDescription>
                  Por valor financeiro impactado • Total: R$ {differencesByType.reduce((s, d) => s + d.value, 0).toLocaleString('pt-BR', {
                minimumFractionDigits: 2
              })}
                </CardDescription>
              </CardHeader>
              <ResponsiveContainer width="100%" height={300}>
                <BarChart data={differencesByType} layout="vertical">
                  ...
                  <XAxis type="number" />
                  <YAxis dataKey="name" type="category" width={140} style={{
                fontSize: 12
              }} />
                  <ChartTooltip formatter={(value: number) => `R$ ${value.toLocaleString('pt-BR', {
                minimumFractionDigits: 2
              })}`} contentStyle={{
                fontSize: 12
              }} />
            ...
            </Card> : <Card className="p-6 flex items-center justify-center h-[400px]">
      ...
      <section>
        ...
        {timelineData.length > 0 && <Card className="p-6 mb-6">
            <CardHeader className="p-0 mb-4">
              <CardTitle className="text-lg">Evolução: Previsto vs Recebido</CardTitle>
              <CardDescription>
                Diferença média do período: {avgDifference}% • Total de diferenças: R$ {totalDiferencas.toLocaleString('pt-BR', {
              minimumFractionDigits: 2
            })}
              </CardDescription>
            </CardHeader>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <Card className="p-6">
            ...
            {recurringDifferences.length > 0 ? <div className="space-y-3">
                {recurringDifferences.map((diff, idx) => <div key={idx} className="flex items-center justify-between p-3 bg-muted/30 rounded-lg border border-border/50">
                    <div className="flex-1">
                    ...
                    <div className="text-right">
                      <p className="text-sm font-bold text-foreground">
                        R$ {diff.value.toLocaleString('pt-BR', {
                    minimumFractionDigits: 2
                  })}
                      </p>
                    </div>
                  </div>)}
              </div> : <p className="text-sm text-muted-foreground text-center py-8">Nenhuma diferença recorrente detectada</p>}
          </Card>
          ...
          <Card className="p-6 bg-gradient-to-br from-primary/5 to-blue-500/5 border-primary/20">
            ...
            <div className="space-y-3">
              {reconciliationRate >= 80 && <p className="text-sm text-foreground flex gap-2">
              ...
              {recoveredValue > 0 && <p className="text-sm text-foreground flex gap-2">
                  <span className="text-success">💰</span>
                  <span><strong>R$ {recoveredValue.toLocaleString('pt-BR', {
                    minimumFractionDigits: 2
                  })} recuperados</strong> — valores que não apareceriam sem a conciliação automática.</span>
                </p>}
              {timeSaved > 5 && <p className="text-sm text-foreground flex gap-2">
                  <span className="text-primary">⚡</span>
                  <span><strong>{timeSaved} horas economizadas</strong> neste período. Tempo que você pode usar para crescer o negócio.</span>
                </p>}
              {Number(avgDifference) > 5 && <p className="text-sm text-foreground flex gap-2">
              ...
              {confirmedCostValue > 1000 && <p className="text-sm text-foreground flex gap-2">
                  <span className="text-danger">📊</span>
                  <span><strong>R$ {confirmedCostValue.toLocaleString('pt-BR', {
                    minimumFractionDigits: 2
                  })} em custos confirmados</strong>. Use esses dados para negociar melhores condições.</span>
                </p>}
            </div>
          </Card>
        </div>
      ...
      <section>
        ...
        <div className="grid md:grid-cols-3 gap-4">
          {totalWithDifference > resolvedCount && <Card className="p-6 bg-warning-light border-warning-border hover:shadow-lg transition-shadow cursor-pointer group">
              <div className="flex items-start gap-3">
          ...
          {supportOpenValue > 0 && <Card className="p-6 bg-orange-500/10 border-orange-500/20 hover:shadow-lg transition-shadow cursor-pointer group">
              <div className="flex items-start gap-3">
          ...
          {confirmedCostValue > 500 && <Card className="p-6 bg-danger-light border-danger-border hover:shadow-lg transition-shadow cursor-pointer group">
              <div className="flex items-start gap-3">
